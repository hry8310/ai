...
class MelDecoder(nn.Module):
    ...
    def forward(self, memory, decoder_input, c_mask, pos):
        ...
        if self.training:
            ...
            mask = mask + t.triu(t.ones(decoder_len, decoder_len).to(dev), diagonal=1).repeat(batch_size, 1, 1).byte()
            ...
            mask = t.triu(t.ones(decoder_len, decoder_len).to(dev), diagonal=1).repeat(batch_size, 1, 1).byte()
