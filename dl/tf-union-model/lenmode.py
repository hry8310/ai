...
class CNN(object):
    def __init__(self, n_classes, img_height, img_width, img_channel, device_name='/cpu:0'):
        ...
        with tf.device(device_name):
            ...
            self.h_pool_4=self.layer(4,self.h_pool_3,f_s=[3,3,32,64])

            num_total_unit = self.h_pool_4.get_shape()[1:4].num_elements()
            print(' num_total_unit  %d' % num_total_unit)
            self.h_pool_3_flat = tf.reshape(self.h_pool_4, shape=[-1, num_total_unit])

            with tf.name_scope('fc_layer_1'):
                self.h_fc_1 = self.fc_layer(self.h_pool_3_flat, num_total_unit, 256, activation_function=tf.nn.relu)

            with tf.name_scope('dropout'):
                 self.h_drop = tf.nn.dropout(self.h_fc_1, keep_prob=self.dropout_keep_prob, name='h_drop')

            with tf.name_scope('fc_layer_2'):
                self.h_fc_2 = self.fc_layer(self.h_drop, 256, 64, activation_function=tf.nn.relu)

            with tf.name_scope('dropout'):
                 self.h_drop_2 = tf.nn.dropout(self.h_fc_2, keep_prob=self.dropout_keep_prob, name='h_drop_2')

            with tf.name_scope('fc_layer_2'):
                self.output = self.fc_layer(self.h_drop_2, 64, n_classes, activation_function=None)
