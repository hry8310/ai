...
class CNN(object):
    def __init__(self, n_classes, img_height, img_width, img_channel, device_name='/cpu:0'):
        self.input_x = tf.placeholder(dtype=tf.float32, shape=[None, img_height//2, img_width//2, img_channel], name='input_x')
        self.input_y = tf.placeholder(dtype=tf.float32, shape=[None, n_classes], name='input_y')
        self.dropout_keep_prob = tf.placeholder(dtype=tf.float32, name='dropout_keep_prob')

        with tf.device(device_name):
            self.input_image = tf.reshape(self.input_x, [-1,img_height//2,img_width//2,img_channel])
