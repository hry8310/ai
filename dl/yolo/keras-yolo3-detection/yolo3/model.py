...
def box_giou( _x, _y):
    ...
    _x = tf.concat([_x[..., :2] - _x[..., 2:] * 0.5,
                        _x[..., :2] + _x[..., 2:] * 0.5], axis=-1)
    _y = tf.concat([_y[..., :2] - _y[..., 2:] * 0.5,
                        _y[..., :2] + _y[..., 2:] * 0.5], axis=-1)

    _x = tf.concat([tf.minimum(_x[..., :2], _x[..., 2:]),
                        tf.maximum(_x[..., :2], _x[..., 2:])], axis=-1)
    _y = tf.concat([tf.minimum(_y[..., :2], _y[..., 2:]),
                        tf.maximum(_y[..., :2], _y[..., 2:])], axis=-1)

    Ag = (_x[..., 2] - _x[..., 0]) * (_x[..., 3] - _x[..., 1])
    Ap = (_y[..., 2] - _y[..., 0]) * (_y[..., 3] - _y[..., 1])

    _It = tf.maximum(_x[..., :2], _y[..., :2])
    _Id = tf.minimum(_x[..., 2:], _y[..., 2:])

    _Ib = tf.maximum(_Id - _It, 0.0)
    I = _Ib[..., 0] * _Ib[..., 1]
    U = Ag + Ap - I 
    iou = I / U 

    Ac_t = tf.minimum(_x[..., :2], _y[..., :2])
    Ac_d = tf.maximum(_x[..., 2:], _y[..., 2:])
    Ac_b = tf.maximum(Ac_d - Ac_t, 0.0)
    Ac = Ac_b[..., 0] * Ac_b[..., 1]
    giou = iou - 1.0 * (Ac -U) / Ac 
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    if calc_loss == True:
        return grid, feats, box_xy, box_wh,feats
...
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    ...
    box_xy, box_wh, box_confidence, box_class_probs,_ = yolo_head(
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=True):
    ...
    for l in range(num_layers):
        ...
        grid, raw_pred, pred_xy, pred_wh , _output = \
        ...
        #_giou=tf.expand_dims( box_giou(pred_box,y_true[l][:, :, :, :, 0:4]), axis=-1)
        #_giou=tf.expand_dims( box_giou(_output[..., 0:4],y_true[l][:, :, :, :, 0:4]), axis=-1)
        _giou=tf.expand_dims( box_giou(raw_pred[..., 0:4], K.concatenate([raw_true_xy, raw_true_wh])  ), axis=-1)
        giou_loss = object_mask * box_loss_scale * (1- _giou) 
        giou_mloss= K.sum(giou_loss) / mf
        ...
        loss += giou_mloss+ confidence_loss + class_loss
        if print_loss:
            #loss = tf.Print(loss, [loss, xy_loss, wh_loss, confidence_loss, class_loss, K.sum(ignore_mask)], message='loss: ')
            loss = tf.Print(loss, [loss, giou_mloss, confidence_loss, class_loss, K.sum(ignore_mask)], message='loss: ')
    return loss
