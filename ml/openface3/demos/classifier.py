...
def train(args):
    ...
    labels2 = pd.read_csv(fname, header=None).as_matrix()[:,0]
    print(labels2)
    labels = map(itemgetter(1),
                 map(os.path.split,
                     map(os.path.dirname, labels2)))  # Get the directory.
    fname = "{}/reps.csv".format(args.workDir)
    embeddings = pd.read_csv(fname, header=None).as_matrix()
    le = LabelEncoder().fit(labels2)
    labelsNum = le.transform(labels2)
    ...
    for img in args.imgs:
        ...
        for r in reps:
            ...
            if multiple:
                ...
                print("Predict {%d} with {%f} confidence." %(maxI, confidence))
