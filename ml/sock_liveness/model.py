...
def moving( frame ,out,times,dist):
    ...
    if len(rects) == 1:
        ...
        ret, pitch, yaw, roll =cn.get_rolate_angle_debug(frame,_shape,frame.shape)   
...
def train():
    ...
    while True:
        ...
        if out is None:
            print(cn.outf())
            print(frame.shape)
            out = cv2.VideoWriter(cn.outf(), cv2.VideoWriter_fourcc('m', 'p', '4', 'v'),16.0,(frame.shape[1],frame.shape[0]) )
        times,dist=moving(frame,out,times,dist)        
